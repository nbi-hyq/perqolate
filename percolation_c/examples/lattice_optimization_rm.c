...
/* construct graph by fusing corresponding GHZ-states with photon loss (using Newman-Ziff method)
   remove edges step by step and check if the loss threshold gets lower */
int main(){
  srand(time(NULL));
  ...
  int64_t num_vec_start = 1;
  for(uint8_t j=0;j<dimension;j++) num_vec_start = num_vec_start * (2*(int64_t)max_step + 1);
  num_vec_start -= 1; // -1 because 0-vector is not considered
  num_vec_start /= 2; // if two vectors just differ by sign, just look at one of them

  int repetitions_per_run = 2*num_vec_start; // make larger, if routine stops before reaching local optium
  ...
  int64_t* vecs_best_run = malloc(sizeof(int64_t) * (int64_t)dimension * num_vec_start); // vectors of best run
  ...
  for(int run=0; run<number_of_runs; run++){
    ...
    num_vec = num_vec_start; // start with all vectors
    for(int rep=0; rep<repetitions_per_run; rep++){
      ...
      if(rep == 0 || lambda < lambda_min){ // then: go on with one vector less
        lambda_min = lambda;
        num_vec -= 1;
        cnt_fail = 0;
      }else { // then: remove a different vector and bring back the removed one
        cnt_fail++;
        if(num_vec - cnt_fail < 0){
          free(percolated);
          break; //removing a vector always makes the lattice worse
        }
        int64_t temp[dimension];
        memcpy(temp, vecs + (num_vec - cnt_fail) * (int64_t)dimension, sizeof(int64_t) * (int64_t)dimension);
        memcpy(vecs + (num_vec - cnt_fail) * (int64_t)dimension, vecs + num_vec * (int64_t)dimension, sizeof(int64_t) * (int64_t)dimension);
        memcpy(vecs + num_vec * (int64_t)dimension, temp, sizeof(int64_t) * (int64_t)dimension);
      }
      free(percolated);
    }
    num_vec++; // keep one vector more for final result
    if(lambda_min < lambda_min_best_run){ // save the results
      num_vec_best_run = num_vec;
      memcpy(vecs_best_run, vecs, sizeof(int64_t) * (int64_t)dimension * num_vec_start);
