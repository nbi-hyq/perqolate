...
int main(){
  ...
  */
  ...
  float sweep_prob[SWEEP_LEN] = {0.095, 0.105};
  int64_t* percolated = percolate_bond(&g, 1.0, &idxLambda); // bond-percolation given fixed site probability (Newman-Ziff method)
  float* expectation_value = get_expectation_value(sweep_prob, SWEEP_LEN, percolated, g.num_edges, 0, 1, true, false); // compute a bond-percolation curve
  if(expectation_value[0] > 0.5) r = -1; // below percolation threshold
  if(expectation_value[1] < 0.5) r |= -1; // above percolation threshold
