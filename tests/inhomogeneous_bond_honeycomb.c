...
int main(){
  ...
  /* define unit graph of honeycomb lattice */
  ...
  pBond[0] = 0.5; pBond[1] = 0.75; pBond[2] = 0.80; // start close to criticality for test speed
  ...
  for (int i=0; i<num_step; i++){
    ...
    printf("%.15f %f %f\n", p_ratio, pBond[0], 1 - pBond[0]*pBond[1] - pBond[2]*pBond[1] - pBond[0]*pBond[2] + pBond[0]*pBond[1]*pBond[2]);
  }

  int r = (1 - pBond[0]*pBond[1] - pBond[2]*pBond[1] - pBond[0]*pBond[2] + pBond[0]*pBond[1]*pBond[2] > 0.05); // check criterion from https://doi.org/10.1063/1.1704215
  r |= (1 - pBond[0]*pBond[1] - pBond[2]*pBond[1] - pBond[0]*pBond[2] + pBond[0]*pBond[1]*pBond[2] < -0.005); // check criterion from https://doi.org/10.1063/1.1704215
  free_unit_graph(&u);
  free_graph(&g);
  free(pBond);
  return r;
