...
int main(){
  ...
  // 1 fusion attempt
  fusion_attempts = 1;
  float sweep_prob[2] = {0.98, 1.0}; // 1 - p_loss (tuned)
  int64_t idxLambda;
  ...
  // 2 fusion attempts
  fusion_attempts = 2;
  sweep_prob[0] = 0.95; sweep_prob[1] = 0.99; // 1 - p_loss (tuned)
  largest_component = fusion_repeat_and_loss_nz(&g, 0.5, fusion_attempts, &numQbts, &numStatic, &idxLambda);
  expectation_value = get_expectation_value(sweep_prob, sweep_len, largest_component, numQbts, numStatic, g.nnode, true, false); // compute fusion/loss-percolation curve
  // below percolation threshold
  if(expectation_value[0] > 0.1) return -1;
  ...
  // 3 fusion attempts
  fusion_attempts = 3;
  sweep_prob[0] = 0.945; sweep_prob[1] = 0.985; // 1 - p_loss (tuned)
  largest_component = fusion_repeat_and_loss_nz(&g, 0.5, fusion_attempts, &numQbts, &numStatic, &idxLambda);
  expectation_value = get_expectation_value(sweep_prob, sweep_len, largest_component, numQbts, numStatic, g.nnode, true, false); // compute fusion/loss-percolation curve
  // below percolation threshold
  if(expectation_value[0] > 0.1) return -1;
